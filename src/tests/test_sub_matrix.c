...
START_TEST(random_sub) {
  ...
  ck_assert_int_eq(s21_sub_matrix(&A, &B, &got), OK);
...
START_TEST(zero_columns) {
  ...
  ck_assert_int_eq(s21_sub_matrix(&A, &B, &got), INCORRECT_MATRIX);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
...
START_TEST(different_sizes_1) {
  ...
  ck_assert_int_eq(s21_sub_matrix(&A, &B, &got), CALC_ERROR);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
}
END_TEST
...
Suite *suite_sub_matrix() {
  ...
  tcase_add_loop_test(tc, random_sub, 0, 50);
